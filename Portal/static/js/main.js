    ...
    $(".dynamicTable[data-ajax-endpoint]").each(function(i,v){
        ...
        function generatePagination(info){
            ...
            if(info["current_start"] != 0){
                out += generatePaginationLi("#","<span aria-hidden=\"true\">&laquo;</span><span class=\"sr-only\">Previous</span>"," aria-label=\"Previous\" data-page=\"0\"");
            }
            ...
            for (var i = pageStart; i < Math.min(pageCount, current_page + pageCountRange); i++){
                out+=generatePaginationLi("#", (i+1) + (i == current_page ? "<span class=\"sr-only\">(current)</span>":"")," data-page=\""+page+"\"", i==current_page? "page-item active": "page-item");
                page+=info["per_page"];
            }
            ...
            if(current_page + 1 < pageCount){
                out += generatePaginationLi("#","<span aria-hidden=\"true\">&raquo;</span><span class=\"sr-only\">Next</span>"," aria-label=\"Next\" data-page=\""+((current_page + 1 )*info["per_page"])+"\"");
            }
        ...
        if ($(v).attr("data-ajax-refresh-time")) {
            setInterval(function(){ updateContent(v, $(v).attr("data-ajax-endpoint")) }, $(v).attr("data-ajax-refresh-time"));
        }
    });
